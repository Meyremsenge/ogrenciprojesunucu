...
# =============================================================================
# GPT AI SERVICE - PRODUCTION READY
# =============================================================================

class GPTAIService(AIServiceInterface):
    """
    OpenAI GPT API servisi - Production Ready.
    ...
    def __init__(self):
        ...
        # Production client'ı import et
        from app.services.ai_client import get_ai_client
        self._client = get_ai_client()
    
    def _call_api(self, messages: List[Dict], max_tokens: int = None) -> Dict[str, Any]:
        """
        OpenAI API çağrısı - production-ready.
        ...
        response = self._client.chat(
            ...
            fallback_to_mock=True  # Hata durumunda mock'a düş
        )
        
        if not response.success:
            # Fallback olarak mock yanıt döndür
            return {
                'content': f"[AI geçici olarak kullanılamıyor. Lütfen daha sonra tekrar deneyin.]",
                'tokens_used': 0,
                'cost_usd': 0,
                'is_mock': True,
    ...
    def generate_hint(self, question: str, hint_level: int, subject: str = None) -> Dict[str, Any]:
        """Soru için ipucu üret."""
        system_prompt = """Sen bir eğitim asistanısın. Öğrencilere sorularını çözmeleri için 
        ipuçları veriyorsun. Doğrudan cevabı VERME, sadece düşünme sürecine yardımcı ol.
        
        İpucu seviyesine göre:
        - Seviye 1: Çok genel bir yönlendirme
        - Seviye 2: Kullanılacak formül veya kavram
        - Seviye 3: Adım adım yaklaşım (cevap hariç)"""
        
        user_prompt = f"""Soru: {question}
        
        İpucu Seviyesi: {hint_level}
        Konu: {subject or 'Genel'}
    ...
    def explain_topic(self, topic: str, difficulty: str, learning_style: str = None) -> Dict[str, Any]:
        ...
        style_instructions = {
            'visual': 'Görsel öğeler, diyagramlar ve benzetmeler kullan.',
            'auditory': 'Konuşma dili kullan, adım adım anlat.',
            'kinesthetic': 'Pratik örnekler ve uygulamalar üzerinden anlat.',
            'reading': 'Detaylı metin açıklamaları ver, kaynaklar öner.'
        }
        
        system_prompt = f"""Sen deneyimli bir öğretmensin. Konuları {difficulty} seviyesinde anlatıyorsun.
        {style_instructions.get(learning_style, 'Açık ve anlaşılır bir dil kullan.')}"""
        
        user_prompt = f"'{topic}' konusunu {difficulty} seviyesinde açıkla."
    ...
    def create_study_plan(self, user_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        system_prompt = """Sen bir eğitim danışmanısın. Öğrencinin bilgilerine göre 
        kişiselleştirilmiş ve gerçekçi çalışma planları oluşturuyorsun.
        
        Plan şunları içermeli:
        - Günlük çalışma süresi
        - Haftalık hedefler
        - Konu sıralaması (zayıf konular önce)
        - Motivasyon önerileri"""
        
        user_info = f"""
        Hedef: {user_data.get('goal', 'Genel başarı')}
        Zayıf Konular: {', '.join(user_data.get('weak_topics', ['Belirtilmedi']))}
        Güçlü Konular: {', '.join(user_data.get('strong_topics', ['Belirtilmedi']))}
        Günlük Çalışma Süresi: {user_data.get('daily_hours', 2)} saat
        Hedef Tarih: {user_data.get('target_date', 'Belirtilmedi')}
        """
        
        response = self._call_api([
            {'role': 'system', 'content': system_prompt},
            {'role': 'user', 'content': f"Bu öğrenci için çalışma planı oluştur:\n{user_info}"}
        ], max_tokens=1000)
    ...
    def evaluate_answer(self, question: str, answer: str, rubric: Dict = None) -> Dict[str, Any]:
        """Cevabı değerlendir."""
        system_prompt = """Sen bir öğretmensin. Öğrenci cevaplarını değerlendiriyorsun.
...
Bu cevabı değerlendir."""
    ...
    def analyze_performance(self, performance_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        system_prompt = """Sen bir eğitim analisti olarak öğrenci performansını değerlendiriyorsun.
        
        Analiz şunları içermeli:
        - Güçlü ve zayıf alanlar
        - Trend analizi (gelişim/gerileme)
        - Öneriler
        - Motivasyon mesajı"""
        
        data_summary = f"""
        Toplam Soru: {performance_data.get('total_questions', 0)}
        Doğru: {performance_data.get('correct', 0)}
        Yanlış: {performance_data.get('incorrect', 0)}
        Ortalama Süre: {performance_data.get('avg_time', 0)} saniye
        Son Performans Trendi: {performance_data.get('trend', 'Stabil')}
        Konu Bazlı Başarı: {performance_data.get('by_topic', {})}
        """
        
        response = self._call_api([
            {'role': 'system', 'content': system_prompt},
            {'role': 'user', 'content': f"Bu performans verilerini analiz et:\n{data_summary}"}
        ], max_tokens=600)
    ...
    def generate_questions(self, topic: str, difficulty: str, count: int, q_type: str) -> Dict[str, Any]:
        """Soru üret."""
        type_instructions = {
            'multiple_choice': '4 seçenekli çoktan seçmeli sorular oluştur. Doğru cevabı belirt.',
            'true_false': 'Doğru/Yanlış soruları oluştur.',
            'open_ended': 'Açık uçlu sorular oluştur. Beklenen cevap formatını belirt.',
            'fill_blank': 'Boşluk doldurma soruları oluştur.'
        }
        
        system_prompt = f"""Sen bir soru yazarısın. {difficulty} seviyesinde, 
        {type_instructions.get(q_type, 'çeşitli tipte')} sorular oluşturuyorsun.
        ...
        response = self._call_api([
            {'role': 'system', 'content': system_prompt},
            {'role': 'user', 'content': f"'{topic}' konusunda {count} adet soru oluştur."}
    ...
    def enhance_content(self, content: str, content_type: str, options: Dict) -> Dict[str, Any]:
        ...
        type_instructions = {
            'summary': 'Bu içeriğin özetini çıkar.',
            'simplify': 'Bu içeriği daha basit bir dille yeniden yaz.',
            'expand': 'Bu içeriği detaylandır ve örnekler ekle.',
            'quiz': 'Bu içerikten quiz soruları oluştur.'
        }
        
        instruction = type_instructions.get(content_type, 'Bu içeriği geliştir.')
